 ...
 * tolerancia... el sistema le debe informar y todo se debe poder ingresar
 * para hacer una correcta medición." Un solo panel: tarifa, adelantos,
 * horas de otras actividades, y generar/gestionar liquidaciones.
 */
export function TeacherPayrollManager({ teachers, courses }: { teachers: any[]; courses: any[] }) {
  ...
  async function refreshAll(id: string) {
    if (!id) return;
    try {
      const [r, a, l, q] = await Promise.all([
        adminApi.teacherRates(id),
        adminApi.teacherAdvances(id),
        adminApi.teacherActivityLogs(id),
        adminApi.teacherLiquidations(id),
      ]);
      setRates(r);
      setAdvances(a);
      setActivityLogs(l);
    ...
    <div className="flex flex-col gap-6">
      ...
      {teacherId && (
        <>
          ...
          <LiquidationEditor teacherId={teacherId} liquidations={liquidations} busy={busy} run={run} />
        </>
      )}
    </div>
  );
...
function LiquidationEditor({ teacherId, liquidations, busy, run }: { teacherId: string; liquidations: any[]; busy: boolean; run: any }) {
  ...
  return (
    <Card>
      <CardContent className="flex flex-col gap-4 p-6">
        <h2 className="font-serif text-lg font-semibold text-ink-900">Liquidaciones</h2>
              ...
              <tbody className="divide-y divide-paper-border">
                {liquidations.map((l) => (
                  <tr key={l.id}>
                    ...
                    <td className="p-2">{currencySymbol(l.currency)}{Number(l.grossAmount).toFixed(2)}</td>
                    <td className="p-2">
                      {currencySymbol(l.currency)}{Number(l.deductions).toFixed(2)} {l.deductionsWaived && <Badge variant="warning">Perdonado</Badge>}
                    </td>
                    <td className="p-2">{currencySymbol(l.currency)}{Number(l.advancesDeducted).toFixed(2)}</td>
                    <td className="p-2 font-medium">{currencySymbol(l.currency)}{Number(l.netAmount).toFixed(2)}</td>
                    <td className="p-2">
        ...
        <div className="flex flex-wrap items-end gap-2 border-t border-paper-border pt-4">
          <div>
            <Label>Desde</Label>
          ...
          <Button
            ...
            onClick={() => run(() => adminApi.generateTeacherLiquidation({ teacherId, periodStart, periodEnd }))}
          >
            Generar liquidación
          </Button>
        </div>
